...
- What is the AWS OpenTelemetry Collector and how to setup it up to ingest and export tracing data to AWS X-Ray.
- Build a full demo to show you how to configure properly a .NET app that sends the tracing data to the Collector.
...
The main component of the AWS Distro for OpenTelemetry is the AWS OTEL Collector which is capable of ingesting metrics and traces sent by the apps and export them into multiple AWS solutions.
...
In our demo the apps will send the tracing data to the OpenTelemetry Collector, and the Collector will export the data to X-Ray.

The Collector is also capable of ingesting metric data in OpenTelemetry format and send it to AWS Cloudwatch Metrics, but in this post I'll be focusing only on tracing data.

It's worth mentioning that for the demo we are not going to use the OpenTelemetry Collector, instead of that we will be using the AWS OpenTelemetry Collector.   
...
I could use the OpenTelemtry Collector with the AWS exporters and it would work, but using the AWS OTEL Collector is simpler and faster to setup, so I decided to ran with it for this demo.
...
We are going need the following ones:
- **Receivers**:
  - **OTEL gRPC Receiver**: The .NET apps will send the tracing data to the collector, and to achieve it we're going to use the OTLP gRPC receiver. This receiver receives data via gRPC using OTLP format.    
  We could use the OTEL HTTP receiver, but the gRPC one is more performant. The HTTP receiver instead of using gRPC receives data via HTTP in JSON format.
- **Exporters**:
...
If you're curious about the logging exporter, here's an example of how the output of the exporter looks like:

![otel-collector-logging-output](/img/aws-otel-collector-logging-output.png)

And here's how the Collector configuration file looks like:
...
In the previous section we talked about what is the AWS OTEL Collector and how to set it up, now it's time to focus on building some apps.
