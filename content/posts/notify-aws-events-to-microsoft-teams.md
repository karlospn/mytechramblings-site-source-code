...
description: "An AWS event indicates a change in a service. In this post I want to show you how you can notify those events to a Microsoft Teams channel using AWS EventBridge and AWS Lambda."
draft: true
---
...
Here's the full list of AWS services that generates events that EventBridge default bus can receive:
...
If you're using IaC to create those ``EventBridge Rules``, the easiest way to do know what values use for the ``EventPattern`` attribute is using first the AWS Portal to create the ``EventPattern`` expression and then copy it into your IaC files.
...
def lambda_handler(event, context):   
    try:
        ...
        logger.error(f"Server connection failed: {err.reason}")
```
The Adaptative Card built inside this Lambda function is quite simple, you can do more complex and interesting stuff if you want, but nonetheless if we take a look now at the end result on Teams:

![teams-adaptative-card](/img/teams-adaptative-card.png)

Now it has a better readability.
...
To execute it by yourself, this is want you need to know.
