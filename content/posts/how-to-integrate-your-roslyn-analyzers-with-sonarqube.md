...
description: "Right now there are 2 ways to add Roslyn Analyzer rules into SonarQube, you can either create a Java plugin from the Roslyn Analyzer and install it into your SonarQube server or import into SonarQube the issues report generated by the Roslyn Analyzer library. In this post I want to show you how these 2 integration options work and also I will talk about the pros and cons of one option over the other."
draft: true
---
...
What are Roslyn Analyzers used for? Roslyn analyzers use the .NET Compiler SDK (Roslyn) to analyze your app source code to find issues and suggest corrections.   
...
Starting in .NET 5, some Roslyn Analyzers packages are included with the .NET SDK, which means that when you create a new .NET app, you'll see that it comes with quite a few of them already preconfigured from the get-go.

If we use Visual Studio to take a look at which Roslyn Analyzers comes with a newly create .NET 7 API, this is what you'll see:
...
Imagine that we've come up with our own set of coding rules for our application, we've built a Roslyn Analyzer library that implements those rules and now we wanna to integrate them with SonarQube, so that issues detected by those rule can be reported and managed in SonarQube.   
What available options do we have if we want to integrate a Roslyn analyzer library with SonarQube? 

Nowadays there are 2 ways to add your Roslyn Analyzer rules to SonarQube:

- Create a Java plugin from the Roslyn Analyzer library and install it into SonarQube.
- Import into SonarQube the issues report generated by the Roslyn Analyzer library.

**And in this post I want to show you how to build these 2 SonarQube integrations, and also talk about the pros and cons of one integration over the other.**
